...
-- PostgREST resolves function signatures by named argument order in RPC calls.
-- Keep callable signature as (p_items, p_quote) for Supabase RPC compatibility.

create or replace function create_quote_transactional(p_items jsonb, p_quote jsonb)
returns jsonb
language plpgsql
as $$
declare
